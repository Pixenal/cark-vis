...
void overrideWrite(
	...
	if (override.override) {
		...
		pixioByteArrWrite(pAlloc, pByteArr, &val, sizeof(val) * 8 - 1);
	}
}

static
bool refOverrideHasOverride(const CarkRefOverride *pRef) {
	return
		pRef->stageIdx.override || pRef->structIdx.override ||
		pRef->compIdx.override || pRef->inst.override;
}
...
PixErr carkOutLogComp(
	...
	if (hasRefOverride) {
		for (I32 i = 0; i < overrideCount; ++i) {
			...
			PIX_ERR_RETURN_IFNOT_COND(
				err,
				refOverrideHasOverride(pRef),
				"ref override doesn't override anything"
			)
...
PixErr carkOutLogEnd(CarkLog *pLog) {
	...
	if (pLog->hasOverrides) {
		...
		*(U32 *)&pInstLog->overrides.pArr[pLog->overrideStart] = header;
		...
		pInstLog->overrides.nextBitIdx = 0;
	}
	++pInstLog->count;
...
void outInstIterUpdate(OutInstIter *pIter) {
	...
	for (I32 i = pIter->thread; i < pIter->pCtx->threadCount; ++i) {
		...
		if (pIter->instLocal >= pStructLog->instArr.count) {
...
PixErr compileInstLogs(
	...
	I32 instIdx = pStage->threadLocal ? pInstIter->inst : pInstIter->instLocal;
	pixioByteArrWrite(pAlloc, pStageData, &instIdx, BITLEN(LOG_INST_COUNT));
	...
	for (I32 i = 0; i < count; ++i) {
		...
		if (pOverrideStart[0]) {
			//has ref override(s)
			I32 bitSize = ((I32)(*(U32 *)pOverrideStart >> 1) - sizeof(U32)) * 8;
			pOverrideStart += sizeof(U32);
			PIX_ERR_ASSERT(
				"override marked true but no overrides exist",
				!pixalcMemIsZero(pOverrideStart + 2, bitSize - 16)
			);
			pixioByteArrWrite(pAlloc, &overrideBuf, &(bool){true}, 1);
			pixioByteArrAlign(&overrideBuf);
			pixioByteArrWrite(pAlloc, &overrideBuf, pOverrideStart, bitSize);
			pixioByteArrAlign(&overrideBuf);
		}
		else {
...
PixErr compileStructLogs(
	...
	*(I32 *)&pStageData->pArr[instCountStart] = instCount;
	return err;
}
...
void stageLogDestroy(CarkOutCtx *pCtx, CarkStageLog *pStageLog) {
	for (I32 i = 0 ; i < pStageLog->structCount; ++i) {
		...
		pStageLog->pStructArr[i].instArr.count = 0;
	}
}
...
PixErr carkOutStageEnd(CarkOutCtx *pCtx, I32 stageIdx, bool compress) {
	...
	for (I32 i = 0 ; i < pStage->structCount; ++i) {
		I32 itemTotalPre = itemTotal;
		I64 bytePre = stageData.byteIdx;
		...
		if (itemTotal == itemTotalPre) {
			stageData.byteIdx = bytePre;
			continue;
		}
		PIX_ERR_ASSERT("", itemTotal >= itemTotalPre);
		++structsLogged;
	}
...
void decodeOverride(PixioByteArr *pBuf, CarkOverride *pOverride) {
	...
	I16 val = 0;
	if (hasOverride) {
		pixioByteArrRead(pBuf, &val, 15);
		val = val == 0x7fff ? -1 : val;
	}
	if (pOverride) {
...
PixErr decodeRefOverrides(
	...
	for (I32 i = 0; i < itemCount; ++i) {
		...
		*pEntry = (CarkOverrideIdx){.idx.idx = offset + 1, .headerIdx = pHeader->byteIdx};
...
PixErr inInstRangeLoad(
	...
	PIX_ERR_ASSERT("", rangeCount <= pInstLog->count);
	I32 offset = 0;
	for (I32 j = 0; j < rangeCount; ++j) {
		...
		PIX_ERR_ASSERT("", range.start < range.end);
...
static
void inStructLogResize(const PixalcFPtrs *pAlloc, CarkInStructLog *pLog, I32 instIdx) {
	//TODO use a redir table instead of allocating in place like this
	I32 minSize = instIdx + 1;
	if (minSize < pLog->instArr.count) {
		return;
	}
	PIXALC_DYN_ARR_RESIZE(
		CarkInInstLog,
...
PixErr decodeLog(
	...
	PIXALC_DYN_ARR_RESIZE_ZERO(CarkInStructLog, &pCtx->alloc, &pLog->structs, pStage->structCount);
	PIXALC_DYN_ARR_RESIZE_ZERO(CarkOverrideIdx, &pCtx->alloc, &pLog->overrideTable, itemTotal);
	...
	pLog->structs.count = pStage->structCount;
	for (I32 i = 0; i < structsLogged; ++i) {
		I32 structIdx = 0;
		pixioByteArrRead(&pMem->buf, &structIdx, BITLEN(LOG_STRUCT));
		CarkInStructLog *pStructLog = pLog->structs.pArr + structIdx;
		*pStructLog = (CarkInStructLog){.idx = structIdx};
		PIX_ERR_RETURN_IFNOT_COND(
			err,
		...
		I32 instsLogged = 0;
		pixioByteArrRead(&pMem->buf, &instsLogged, BITLEN(LOG_INST_COUNT));
		for(I32 j = 0; j < instsLogged; ++j) {
			I32 instIdx = 0;
...
static
void refMakeAbs(CarkInRef *pRef, CarkInRef active) {
	if (pRef->ref.stageIdx == -1) {
		pRef->ref.stageIdx = active.ref.stageIdx;
	}
	if (pRef->ref.structIdx == -1) {
		pRef->ref.structIdx = active.ref.structIdx;
	}
	if (pRef->ref.compIdx == -1) {
		pRef->ref.compIdx = active.ref.compIdx;
	}
	if (pRef->inst == -1) {
		pRef->inst = active.inst;
	}
}
...
PixErr carkInCompRefsGet(
	...
	I32 overrideIdx = 0;
	for (I32 i = 0; i < pCompInfo->refCount; ++i) {
		CarkInRef *pRefEntry = pRefArr->arr + i;
		pRefEntry->ref = pCompInfo->refArr[i];
		pRefEntry->inst = ref.inst;

		CarkRefOverride *pOverride = overrideArr.arr + overrideIdx;
		PIX_ERR_ASSERT("", pOverride->refIdx < pCompInfo->refCount);
		if (i == pOverride->refIdx) {
			setIfOverridden(&pRefEntry->ref.stageIdx, pOverride->stageIdx);
			setIfOverridden(&pRefEntry->ref.structIdx, pOverride->structIdx);
			setIfOverridden(&pRefEntry->ref.compIdx, pOverride->compIdx);
			setIfOverridden(&pRefEntry->inst, pOverride->inst);
		}
		refMakeAbs(pRefEntry, ref);
	}
